...
import {LunaProjectHelper, IPluginDetails} from './LunaProjectHelper';
...
export class TelemetryHelper {
  ...
  public static sendPluginsList(projectRoot: string, pluginsList: string[]): void {
    ...
    if (LunaProjectHelper.existsSync(pluginFilePath)) {
      try {
    ...
    let pluginDetails: IPluginDetails[] =
      newPlugins.map(pluginName => LunaProjectHelper.getInstalledPluginDetails(projectRoot, pluginName))
  ...
  private static setTelemetryEventProperty(event: Telemetry.TelemetryEvent, propertyName: string,
                                           propertyValue: string, isPii: boolean): void {
    if (isPii) {
  ...
  private static addMultiValuedTelemetryEventProperty(event: Telemetry.TelemetryEvent, propertyName: string,
                                                      propertyValue: string[], isPii: boolean): void {
